  ...
  return (
    <div className="overflow-hidden">
      ...
      <section className="relative min-h-[90vh] flex items-center bg-gradient-to-br from-tempe-cream-50 via-tempe-cream-100 to-tempe-green-50/20 pattern-grid pt-12 pb-24">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-16">
            {/* Left Content */}
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="flex flex-col items-start text-left">
              ...
              <p className="mt-6 text-lg font-medium text-tempe-green-800/90 leading-relaxed max-w-lg">{profile?.tagline ?? "Digitalisasi UMKM Perajin Tempe Kelurahan Bencongan"}</p>
            ...
            >
              ...
              <div className="relative w-full max-w-lg overflow-hidden rounded-[2.5rem] bg-stone-100 p-3 shadow-card-hover ring-1 ring-stone-900/5 group">
                ...
                <HeroSlider images={heroImages} alt="Sentra Tempe Bencongan" ratio="aspect-[4/3]" className="rounded-[2.2rem] transition-transform duration-700 group-hover:scale-105" />
              </div>
          ...
          {stats.map((stat) => (
            <div key={stat.label} className="flex flex-col items-center justify-center border-r border-stone-100 last:border-0 px-2">
              ...
              <p className="leading-none">
                <AnimatedNumber value={stat.value} suffix={stat.suffix} suffixClassName="ml-0.5 text-base font-semibold text-tempe-green-600" className="tabular-nums text-3xl font-extrabold text-stone-900 leading-none" />
              </p>
      ...
      <section className="mx-auto max-w-7xl px-4 py-24 sm:px-6 lg:px-8">
        <SectionHeading eyebrow="Daftar Pengusaha" title="UMKM Perajin Tempe Unggulan" subtitle="Para perajin tempe lokal pilihan yang aktif memproduksi tempe higienis dengan bahan kedelai bermutu tinggi setiap hari." />
                ...
                <motion.div key={item.id} variants={cardVariants}>
                  <Card hover className="h-full flex flex-col overflow-hidden">
                    <div className="relative overflow-hidden group">
                      <Photo src={item.imageUrl} alt={item.nama} ratio="aspect-[4/3]" className="rounded-t-none border-b border-stone-150 transition-transform duration-500 group-hover:scale-105" />
                    ...
                    <div className="flex flex-col flex-1 p-6 text-left">
                      <h3 className="text-xl font-bold text-stone-900 tracking-tight leading-snug">{item.nama}</h3>
                      ...
                      <p className="mt-3.5 line-clamp-2 text-sm leading-relaxed text-stone-600 flex-1">{item.deskripsi}</p>

                      <div className="mt-5 pt-4 border-t border-stone-100 flex items-center justify-between">
                        <span className="text-xs font-bold text-tempe-gold-700 bg-tempe-gold-50 px-2.5 py-1 rounded-lg">{item.kapasitas}</span>

                        <Link to={`/umkm/${item.id}`} className="inline-flex items-center gap-1.5 text-sm font-bold text-tempe-green-700 hover:text-tempe-green-950 transition-all">
                          Lihat Detail
      ...
      <section className="bg-gradient-to-b from-stone-50 to-tempe-cream-50 border-y border-stone-200/50 py-24">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <SectionHeading eyebrow="Kabar Terkini" title="Kegiatan & Pemberdayaan Desa" subtitle="Kanal berita resmi mengenai program kerja pembinaan Karang Taruna dan agenda bersama Sentra Tempe." />
              ...
              <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
                {newsPreview.map((item) => (
                  <motion.div key={item.id} variants={cardVariants}>
                    <Card hover className="h-full flex flex-col overflow-hidden">
                      <div className="relative overflow-hidden group">
                        <Photo src={item.coverUrl} alt={item.judul} ratio="aspect-video" className="rounded-t-none border-b border-stone-150 transition-transform duration-500 group-hover:scale-105" />
                      ...
                      <div className="flex flex-col flex-1 p-6 text-left">
                        <span className="text-xs font-bold text-stone-400">{formatTanggalPendek(item.tanggal)}</span>
                        ...
                        <p className="mt-3 line-clamp-2 text-sm leading-relaxed text-stone-600">{item.ringkasan}</p>

                        <Link to={`/kegiatan/${item.id}`} className="mt-5 pt-3 border-t border-stone-100 inline-flex items-center gap-1.5 text-sm font-bold text-tempe-green-700 hover:text-tempe-green-950 transition-all">
                          Baca Selengkapnya
                          <ArrowRight className="h-4.5 w-4.5" aria-hidden />
                        </Link>
                      </div>
          ...
          <Card className="overflow-hidden border-0 bg-gradient-to-br from-tempe-green-800 to-tempe-green-900 shadow-xl relative">
            ...
            <div className="relative grid items-center gap-10 p-8 sm:p-14 lg:grid-cols-[1.2fr_1fr] z-10">
              <div className="text-left">
                ...
                <h2 className="text-3xl font-extrabold text-white sm:text-4xl tracking-tight leading-tight">Tertarik Memesan Tempe Bencongan?</h2>
              ...
              <div className="hidden lg:block relative p-2">
                ...
                <Photo src={heroImages[0] ?? profile?.heroImageUrl} alt="Kemitraan Sentra Tempe" ratio="aspect-video" className="rounded-[1.5rem] shadow-lg border border-white/10" />
              </div>
            </div>
          </Card>
        </motion.div>
